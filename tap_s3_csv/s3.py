...
def get_sampled_schema_for_table(config, table_spec):
    ...
    if selected_properties:
        ...
        LOGGER.info(f"Selecting for only the following properties: {', '.join(selected_properties)}")
        data_schema =  {k: v for k,v in data_schema.items() if k in selected_properties}
